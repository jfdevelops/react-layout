...
- 397763c: `createResourceLinks.withGroups()` no longer generates a random group id
...
- 83b8346: Add `defineResourceLayout.withLayout` for a once-mounted application shell.
...
- bfc559e: Refine the `resources` accessor on `defineResourceLayout` and its `render` context.
...
- c109b97: Add resource access helpers to `defineResourceLayout`.
